...
In \autoref{fig:4-throughput} we observe a significantly lower maximum throughput for all worker thread configurations, namely between 12.5 and 20\% less throughput from 8 up to 64 worker threads. When looking at \autoref{fig:4-times} we observe that the total response time is over 3 times higher compared to the one measured in \autoref{sec3.2}. The service time seems to be similar at first, but when inspected in detail we observe that it is slightly higher.
At the maximum troughput configuration for each worker thread configuration (see \autoref{res4}) a difference of the service time (average time waiting for memcached) compared to \autoref{sec3.2} has been measured of 0.33ms, 0.41ms, 0.9ms and 1.5ms for 8, 16, 32 and 64 worker threads respectively. We also observed that the average time in the queue differs also exactly by this time. The amount of requests in the queue has been observed to be similar compared to \autoref{sec3.2} for each of the worker thread configurations. As each request has to wait approximately $service\_time / \#worker\_threads$.
%hey leute ich han eventuell ein nützlichi iisicht gha (vlt isch au obvious): mer chan d queue time wie folg approximiere (tot ahzal requests ide queue)*(memcached time)/(tot azahl workers)
%wieso? wel es chönd immer nur max. so vil requests wies workers hed glichziitig bem server si.. alli andere requests ide queue mönd warte bis si drachömend.. und sie mönd genau (servicetime)/(workers) lang warte im schnitt
%ich find das no nützlich wel ich han be section 4 gse dass service time zu 3 server anstatt 1 server in section 3 für 8 WT zb nur minim grösser isch... aber dafür isch queue time vil grösser... aber trotzdem heds fast genau glich vil requests ide queue.. aber wemer (# req ide queue) * (service time differenz)/wt rechnet chund mer de grossi gap ide queue waiting time über .. mystery solved
